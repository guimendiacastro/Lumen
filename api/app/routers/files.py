...
"""
File upload and management API with RAG support.
"""
...
# Maximum file size: 30MB (similar to Claude)
...
class RetrievalResponse(BaseModel):
    chunks: List[dict]  # [{file_id, text, similarity}]
    ...
    async with member_session(schema) as s:
        ...
        await s.execute(
            ...
            """),
            {
                ...
                "mime": mime_type,
                "size": size_bytes,
                "path": f"local/{file_id}",  # Placeholder - extend for S3
                "content": content_enc,
                "status": "processing",
                "by": user_id
            }
...
async def _process_and_store_chunks(
    ...
    """Process chunks and store embeddings. Returns chunk count."""
...
async def upload_file(
    ...
    """
    Upload a file and process it for either:
    1. Direct context (small files < 50k chars)
    2. RAG with chunking and embeddings (large files)
    """
    ...
    # Process file
    try:
        result = FileProcessor.process_file(content, file.content_type or "text/plain")
    ...
    # Process chunks in background (for now, synchronous)
    ...
    async with member_session(schema) as s:
        ...
        for row in result:
            files.append(FileMetadata(
                ...
                size_bytes=row[3],
                status=row[4],
                use_direct_context=row[6] == 0,  # No chunks = direct context
                chunk_count=row[6],
...
async def retrieve_relevant_chunks(
    ...
    Used by AI compare endpoint to augment context.
    """
    ...
    # Fetch all chunks and embeddings for specified files
    chunk_data = []
    async with member_session(schema) as s:
        for file_id in body.file_ids:
            result = await s.execute(
                text("""
                    SELECT c.chunk_text_enc, e.embedding_vector, c.id, f.filename
                    FROM file_chunks c
            ...
            for row in result:
                ...
                chunk_data.append((chunk_text, embedding, chunk_id, filename))
    ...
    chunk_embeddings = [(text, emb) for text, emb, _, _ in chunk_data]
    ...
    for relevant_text, similarity in relevant:
        for text, _, chunk_id, filename in chunk_data:
